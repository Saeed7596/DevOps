...
|:---|:---|
| `--control-plane-endpoint` | The shared endpoint (DNS name or IP address) for all control plane nodes. It is used by worker nodes and clients to communicate with the Kubernetes API server. |
| `--apiserver-advertise-address` | The IP address the API server will advertise to other nodes. Typically, it should be the node’s private IP address. |
| `--pod-network-cidr` | The CIDR range for Pod IP addresses. Your CNI plugin must match this range (e.g., `192.168.0.0/16` for Calico). |
| `--service-cidr` | The CIDR range for Kubernetes services' cluster IPs. Default is usually `10.96.0.0/12`. |
| `--apiserver-cert-extra-sans` | Extra Subject Alternative Names (SANs) to add to the API server’s serving certificate (e.g., external IPs, DNS names). Useful for HA or external access. |
| `--upload-certs` | Uploads the control plane certificates to the kubeadm-certs secret in the cluster, allowing new control plane nodes to join securely without manually copying certificates. |
